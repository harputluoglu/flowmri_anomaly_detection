...
if __name__ == "__main__":
    ...
    for which_dataset in config['which_datasets']:
        ...
        for i in subject_indexes:
            ...
            while end_idx <= config["spatial_size_z"]:
                ...
                batch_slice_info = np.digitize(range(start_idx,end_idx), bins)-1

                feed_dict = {model.image_matrix: subject_sliced[start_idx:end_idx], model.batch_slice_info: batch_slice_info}
    ...
    # Add noise to the image on each channel and change the amount of noise added

    # Compute reconstruction using the model

    # Compute the difference image between the two

    # Compute the RMSE between the added noise image and the difference image as a measure of how well the model did (similar to dice score I guess)
    ...
    for idx in config['noise_subjects_validation']:
        ...
        # loop over the different levels of noise
        for (mean, stdev) in noise_parameters:
            ...
            while end_idx <= config["spatial_size_z"]:
                ...
                # TODO: uncommenting this for now, but might add it again in the future.
                logging.info('{} AUC VALUES: threshold;dice score'.format(model_name))
