...
# =================================================================================
from helpers.batches import iterate_minibatches, iterate_minibatches_with_sliceinfo, iterate_minibatches_with_class
...
if __name__ == "__main__":
    ...
    # ==== If mask preprocessing is enabled, do preprocessing of masked the data now
    # =================================================================================
    if preprocess_enabled == "mask_sliced":
    ...
    #Initialize the reduced conditional VAE
    ...
    # Should we continue training an existing model or is this a new model?
    ...
    for ep in range(already_completed_epochs, epochs):
        ...
        for (batch, batch_slice_info) in iterate_minibatches_with_class(images_tr, 
                                                                        batch_size = batch_size, 
                                                                        number_of_classes = config["number_of_classes"],
                                                                        data_augmentation=config["do_data_augmentation"]):

            input_images = batch
            model.train(input_images= input_images, batch_slice_info= batch_slice_info, weight= weight) 
        ...
        if ep % 10 == 0:
            ...
            gen_loss, res_loss, lat_loss = model.sess.run([model.autoencoder_loss,
                                                           ...
                                                           model.latent_loss], {model.image_matrix: input_images, model.batch_slice_info: batch_slice_info})


            # Write summary for tensorboard
            summary_str = model.sess.run(model.summary_op, {model.image_matrix: input_images,model.batch_slice_info: batch_slice_info, model.weight: weight})
        ...
        if ep % validation_frequency == 0:
            ...
            batchcount = 0
            for (batch, batch_slice_info) in iterate_minibatches_with_class(images_vl, batch_size = batch_size, number_of_classes = config["number_of_classes"]):
                input_images = batch

                gen_loss_valid, res_loss_valid, lat_loss_valid = model.sess.run([model.autoencoder_loss_test,
                                                               model.autoencoder_res_loss_test,
                                                               model.latent_loss_test], {model.image_matrix: input_images, model.batch_slice_info: batch_slice_info})
            ...
            model.validate(input_images, batch_slice_info, weight)
