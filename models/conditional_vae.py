...
class ConditionalVAEModel():
    ...
    def tile_condition_matrix(self, input_or_latent):
        ...
        one_hot = tf.one_hot(self.batch_slice_info, self.config["spatial_size_z"])
        
        # Reshape this vector into the shape of 1x1x1x1xnumber_of_categories aka the number of slices
        reshaped = tf.reshape(one_hot, (self.config["batch_size"],1,1,1,self.config["spatial_size_z"]) )
    ...
    def sample(self, batch_slice_info):
        ...
        z = np.random.normal(0,1,(self.config["batch_size"],
                                  ...
                                  self.config["latent_t"],2176)) # TODO: not sure if this is actually the correct dimension of the thing here.....
